...
class THMProfileScraper:
    __username: str
    __log: FilteringBoundLogger
    __image: Image.Image
    __draw: ImageDraw.ImageDraw

    def __init__(self, log_level: str, username: str) -> None:
    ...
    def __generate_badge(self, profile: Dict[str, str]) -> None:
        ...
        self.__draw_corners(bg_color, card_color, height, width, scale)

        # --- Typography (Attempt to use system fonts, fallback to default) ---
        name_font, stat_label_font, stat_value_font = self.__load_fonts(scale)

        # --- Header Content ---
        self.__render_thm_logo(scale)
        ...
        for i, (label, value, color, icon_url) in enumerate(stats_data):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.__image.paste(icon_img, (x, y + sc(1)), icon_img)
                    text_offset = icon_w + sc(10)
                else:
                    text_offset = 0
    ...
    def __render_avatar(self,
                        ...
                        name_font: FreeTypeFont, profile: dict[str, str], width: int, scale: float):
        def sc(val):
            return int(val * scale)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        text_width = sc(100)  # Fallback

                    avatar_x = width - sc(20) - text_width - sc(10) - avatar_size[0]
                    avatar_y = sc(7)
                    border = max(1, sc(2))
                    self.__draw.ellipse(
                            (avatar_x - border, avatar_y - border, avatar_x + avatar_size[0] + border,
    ...
    def __render_thm_logo(self, scale: float):
        def sc(val):
            return int(val * scale)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if drawing:
                    ...
                    self.__image.paste(logo_img, (sc(20), sc(11)), logo_img)
                else:
                    self.__log.warn('Failed to load official logo')
        except Exception as e:
            self.__log.warn(f'Failed to load official logo: {e}')

    def __load_fonts(self, scale: float) -> Tuple[FreeTypeFont, FreeTypeFont, FreeTypeFont]:
        def sc(val):
            return int(val * scale)

        font_path = '/System/Library/Fonts/SFNS.ttf'
        try:
            name_font = ImageFont.truetype(font_path, sc(20))
            stat_label_font = ImageFont.truetype(font_path, sc(14))
            stat_value_font = ImageFont.truetype(font_path, sc(16))
        except Exception:
    ...
    def run(self) -> None:
        self.__log.info('scraping profile')
...
def command_line(log_level: str, username: str) -> None:
    """
    THM profile scraper.\n
    USERNAME = THM profile username. \n
    usage: \n
    python scrap_thm_profile.py <username> --log_level <level>
    """

    print(f'=== Start {THMProfileScraper.__name__} ===')
    runner = THMProfileScraper(log_level, username)
    runner.run()
    print(f'=== End {THMProfileScraper.__name__} ===')
